...
for ind=1:length(values)
    ...
    if strcmp(names{ind+1},'feature_result')
        indice=ind;
        feature_result=values{indice};
        %data_to_save=[data_to_save {'features_results'}];
        data_to_save{indice}={'feature_result'};
        
    end
    % Get performances_results
    if strcmp(names{ind+1},'performance_result')
        indice=ind;
        performance_result=values{indice};
        %data_to_save=[data_to_save {'performances_results'}];
        data_to_save{indice}={'performance_result'};
...
if (isempty(pathstr) < 0.5);
    ...
    %msgbox('Your session is succefully saved','SIGMA infos')
