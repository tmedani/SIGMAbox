...
if length(ic_output)==number_sample
    display('SIGMA>> Your input are correct...')
else
    error('SIGMA>> The size of the feature matrix & labels are not correct (transpose matrix?)...')
    %return
end


%number_probe = max(number_feature, 13); % 13 is the statistical computed value in the literature, Ask Gerard to confirm,  
% number_probe = max(number_feature, 100);
% number_probe = round(number_feature/4);
number_probe = number_feature;
...
output_vector = ic_output - mean(ic_output);% / (norm(ic_output)); %% bizaare non ?pkoi normalis
...
% matrix_probe = normalized_feature;%ic_feature;%normalized_feature;
% for l_feature = 1 : number_feature,
%     label_noise = randperm(number_sample);
...
% feature_probe = [normalized_feature, matrix_probe];
%feature_probe = [normalized_feature, rand(size(ic_feature,1),number_probe)];
...
while selecting == 1,
    ...
    if index <= number_feature,
        ...
        feature_probe = feature_probe(:, [1 : index - 1 index + 1 : end]); 
        indexing = indexing(:, [1 : index - 1 index + 1 : end]); 
        number_probe = number_probe - 1;
    end
    ...
    display('*****************************************')
    display(['number_sample = ' num2str(number_sample)])
    display(['number_feature = ' num2str(number_feature)])
    display(['number_probe = ' num2str(number_probe)])
    display(['ic_threshold = ' num2str(ic_threshold)])

    display(['number_pass = ' num2str(number_pass)])
    display(['number_probe_ranked = ' num2str(number_probe_ranked)])
    display(['number_probe_ranked / number_probe  = ' num2str(number_probe_ranked / number_probe )])

%% condition initiale proposé par FV
%     if (number_pass > number_sample) || (number_pass > number_feature) || ...
%             (number_probe_ranked / number_probe >= ic_threshold)
%         selecting = 0;
%     end
    
     if (number_probe_ranked / number_probe >= ic_threshold)
        selecting = 0;
    end
