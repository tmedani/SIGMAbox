...
for ind=1:size(index_selected,1)
    ...
    index = length(mat_sort);
    while 1
        if ~ismember(mat_sort(index,2),best_voted_index)    
            best_voted_index(ind)=(mat_sort(index,2));
            break
        else
            index = index-1;
            if index == 0
...
%best_voted_index =  unique(best_voted_index,'stable');
end