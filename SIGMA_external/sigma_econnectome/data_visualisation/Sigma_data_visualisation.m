function varargout =Sigma_data_visualisation(varargin)
% Sigma_data_visualisation - the main Graphical User Interface (GUI) environment
...
% Usage:
%     1. type
%         >> Sigma_data_visualisation
%        or call Sigma_data_visualisation to start the popup GUI
%
%     2. type
%         >> Sigma_data_visualisation(EEG)
%         or call Sigma_data_visualisation(EEG) to start the popup GUI with EEG structure.
%         The EEG structure should be pre-exported by the Sigma_data_visualisation GUI
%         or made by
%         >> EEG = pop_txtreader
%         or
%         >> EEG = pop_matreader
...
%
%      3. call Sigma_data_visualisation from the main econnectome GUI ('Menu bar -> EEG')
%
% Description:
%  Sigma_data_visualisation is the main GUI for EEG functional connectivity. Multi-channel EEG
...
%
% Reference for Sigma_data_visualisation() (please cite):
...
% Estimation of the cortical functional connectiv0ity with the multimodal integration of high-resolution
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @Sigma_data_visualisation_OpeningFcn, ...
    'gui_OutputFcn',  @Sigma_data_visualisation_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Sigma_data_visualisation is made visible.
function Sigma_data_visualisation_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Sigma_data_visualisation (see VARARGIN)


% Choose default command line output for Sigma_data_visualisation
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Sigma_data_visualisation wait for user response (see UIRESUME)
...
init_parameter_sigma_eegfc=handles.input.init_parameter;

% init_parameter_sigma_eegfc = varargin{1};
%init_parameter_sigma_eegfc=evalin('base', 'init_parameter');
...
if ~isempty(init_parameter_sigma_eegfc.subject)
    init_parameter_sigma_eegfc.subject_to_display = init_parameter_sigma_eegfc.subject;
    subject = init_parameter_sigma_eegfc.subject_to_display;
else
    subject = init_parameter_sigma_eegfc.subject_index(1);
    init_parameter_sigma_eegfc.subject_to_display=subject;

    init_parameter_sigma_eegfc.subject = init_parameter_sigma_eegfc.subject_index;
end

% Check if the subjec is empty, in this case put the first one to display
% it, this proble is observed when data vis in lunched from GUI aurelien
% if isempty(init_parameter_sigma_eegfc.subject)
%     init_parameter_sigma_eegfc.subject = init_parameter_sigma_eegfc.subject_index;
% end


if sum(init_parameter_sigma_eegfc.subject == subject)
    %We convert the signal
%     if isfield(init_parameter_sigma_eegfc,'subject_to_display')
%         list_sub = [ init_parameter_sigma_eegfc.subject ];
%         if isempty((subject == list_sub))
%             msgbox('There is error in the input subject/epoch, please correct')
%             return;
%         end
%     end
    ...
    msgbox('The selected subject does not exist in your data base (init_parameter)!','','help','modal');
    return;
end
...
nonEEG_obj = findobj(gcf,'tag','bad_channel_text');
...
function varargout = Sigma_data_visualisation_OutputFcn(hObject, eventdata, handles)
...
bad = findobj(gcf,'tag','bad_channel_text');
bad_str=bad.String;
bad_num=[];
...
sub = init_parameter_sigma_eegfc.subject_to_display;
list_sub=[init_parameter_sigma_eegfc.subject];
if isempty(find(sub==list_sub))
    msgbox('There is error in the input subject/epoch, please correct')
    return;
end
...
% Check if the subjec is empty, in this case put the first one to display
% it, this proble is observed when data vis in lunched from GUI aurelien
if length(init_parameter_sigma_eegfc.subject)==0
    init_parameter_sigma_eegfc.subject=init_parameter_sigma_eegfc.subject_index;
end
init_parameter_sigma_eegfc.subject=init_parameter_sigma_eegfc.subject_index;
