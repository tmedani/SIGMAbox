function features_results = Synchro_time_cronbach_alpha(init_parameter,~,features_results,s_EEG,Nsubj,Nepochs,Nmethode)
%%%------------------------------------------------------------------------
...
%%%------------------------------------------------------------------------
o_synchro_time_cronbach_alpha_ch=[];
temp0=[];
if (Nepochs==1)
    o_synchro_time_cronbach_alpha_epo=[];
    if Nsubj==1
        features_results.o_synchro_time_cronbach_alpha=[];
    end
end
    ...
    for l_channel2 = l_channel1+1:size(s_EEG.data,1)
        ...
        if (apply_filter==1)
            for l_band=1:init_parameter.nb_bands
                ...
                X = [fdata_1 fdata_2];
                o_synchro_time_cronbach_alpha0 = cronbach(X);
                o_synchro_time_cronbach_alpha_ch=[o_synchro_time_cronbach_alpha_ch; o_synchro_time_cronbach_alpha0];
...
o_synchro_time_cronbach_alpha_epo=o_synchro_time_cronbach_alpha_ch;

features_results.o_synchro_time_cronbach_alpha=[features_results.o_synchro_time_cronbach_alpha o_synchro_time_cronbach_alpha_epo];

%features_results.o_synchro_time_cronbach_alpha=o_synchro_time_cronbach_alpha;

features_results.channels_combinaison=channels_combinaison;
if ((Nsubj==1)&&(Nepochs==1))
    features_results.o_synchro_time_cronbach_alpha_band=temp0;
end
