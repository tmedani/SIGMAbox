...
if ~isempty(init_parameter_eegfc.subject)
    subject = init_parameter_eegfc.subject_to_display(end);
else
    subject = init_parameter_eegfc.subject_index(1);
    init_parameter_eegfc.subject_to_display=subject;
end
init_parameter_eegfc.subject_to_display = init_parameter_eegfc.subject_to_display(end);
%subject = init_parameter_eegfc.subject_to_display;
%list_subject = [init_parameter_eegfc.subject];
list_subject = init_parameter_eegfc.subject_index;
if isempty((subject==list_subject))
    msgbox('There is error in the input subject/epoch, please correct')
    return;
end
EOI=[];
%Loading the reférence for the electrodes positions
...
ext='.mat';
S=int2str(subject);
% update 06/12/2018 Takfa medani
% if length(S) == 1
%     S = ['0' S];
% end
name0 = strcat(S,ext);
...
%the number of channel of the référence
...
nb_max_epoch=size(EEG_2.s_EEG.data,3);
if epoch<=0
   errordlg('This epoch does not exists', 'SIGMA Error');
    return
end
    
if epoch<=nb_max_epoch
...
%détermine wich is the last epoch available
