...
if isstruct(features_results)
    if isfield(features_results,'o_best_features_matrix')
        ...
        SL0 = features_results.o_features_matrix_normalize(ind1,:);
        SW0 = features_results.o_features_matrix_normalize(ind2,:);
    end
...
%% Get the label or the two classes

group0 =features_results.label;
group0=group0(:);
% label_matrix=Sigma_adapt_label(group0)
%[~, new_label]=Sigma_set_label(group0);
%group0=new_label;
...
%      else
     h=legend(['Class  ' num2str(classes(1))],['Class  ' num2str(classes(2))'],'Location','NW','AutoUpdate','off' );
     end
     h.FontSize=20;
    %% Trace grid 
    xdata=h10.XData;
