...
scores=performances_results.scores{2};% 
...
if plot_best_roc==1
    ...
    %[max_auc, index]=max(AUC);
    [Xa,Ya,~,~,OPTROCPT] = perfcurve(features_results.label,scores(index,:),1);
...
if display_conf_matrix_movie==1
    for nbf=1:init_parameter.nb_features 
        ...
        targets=Sigma_adapt_label(features_results.label);% The target, label to predicts
...
if comput_best_confusion_matrix==1
    ...
    [confusion_matrix,~] = confusionmat(features_results.label,performances_results.prediction(index_max_auc,:));
end
...
actual_groups=features_results.label;
